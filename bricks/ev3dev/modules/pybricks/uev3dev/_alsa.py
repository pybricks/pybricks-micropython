...
class Mixer:
    ...
    _selem_get_playback_volume_range = _alsa.func(
        "i", "snd_mixer_selem_get_playback_volume_range", "ppp"
    )
    _selem_set_playback_volume_all = _alsa.func(
        "i", "snd_mixer_selem_set_playback_volume_all", "pl"
    )
    ...
    def __init__(self):
        ...
        try:
            ...
            Mixer._selem_get_playback_volume_range(self._pcm_elem, addressof(min), addressof(max))
...
class PCM:
    ...
    _hw_params_get_period_size = _alsa.func("i", "snd_pcm_hw_params_get_period_size", "ppp")
    ...
    def play(self, sound_file, cancel_token=None):
        err = PCM._hw_params_set_access(self._pcm, self._hp, PCM._ACCESS_RW_INTERLEAVED)
        _check_error(err)
        err = PCM._hw_params_set_format(self._pcm, self._hp, PCM._FORMAT_S16_LE)
        _check_error(err)
        err = PCM._hw_params_set_channels(self._pcm, self._hp, sound_file._channels)
        _check_error(err)
        err = PCM._hw_params_set_rate(self._pcm, self._hp, sound_file._samplerate, 0)
        _check_error(err)
