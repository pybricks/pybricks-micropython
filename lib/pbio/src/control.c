...
void pbio_control_update(pbio_control_t *ctl, uint32_t time_now, pbio_control_state_t *state, pbio_trajectory_reference_t *ref, pbio_dcmotor_actuation_t *actuation, int32_t *control) {
    ...
    // Also define maximum integral value for anti-windup.
    int32_t integral_range = (ctl->settings.actuation_max / ctl->settings.pid_kp) * 2;
    int32_t integral_max = ctl->settings.pid_ki < 1 ? 0 : ctl->settings.actuation_max / ctl->settings.pid_ki * 1000;
    ...
    int32_t torque_proportional = ctl->settings.pid_kp * position_error_used / 1000;
    int32_t torque_derivative = ctl->settings.pid_kd * speed_error / 1000;
    int32_t torque_integral = ctl->settings.pid_ki * integral_error / 1000;
    ...
    int32_t max_windup_torque = ctl->settings.actuation_max + ctl->settings.pid_kp * windup_margin / 1000;
