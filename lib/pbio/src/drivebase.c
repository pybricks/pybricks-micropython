...
pbio_error_t pbio_drivebase_update(pbio_drivebase_t *db) {
    ...
    pbio_control_update(&db->control_distance, time_now, sum, sum_rate, 0, 0, 0, 0, &sum_actuation, &sum_control, &sum_rate_ref, &sum_acceleration_ref);
    pbio_control_update(&db->control_heading, time_now, dif, dif_rate, 0, 0, 0, 0, &dif_actuation, &dif_control, &dif_rate_ref, &dif_acceleration_ref);
