 ...
 *
 * @param [in]  light       The light instance
 * @param [in]  color       The color and brightness
...
pbio_error_t pbio_color_light_on(pbio_color_light_t *light, pbio_color_t color) {
    ...
    return light->funcs->set_hsv(light, color);
}
...
static uint32_t pbio_color_light_breathe_next(pbio_light_animation_t *animation) {
    ...
    pbio_color_t hsv = PBIO_COLOR_ENCODE(breathe_animation_hue, 100,
        breathe_animation_progress < animation_progress_max / 2 ?
...
static uint32_t pbio_color_light_animate_next(pbio_light_animation_t *animation) {
    ...
    const pbio_color_t *cells = light->hsv_cells;
