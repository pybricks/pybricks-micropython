...
/**
 * This simulates a remote device requesting to enable notifications on the nRF
...
void pbio_test_bluetooth_enable_uart_service_notifications(void) {
    ...
    pbio_test_bluetooth_enable_notifications(0x000d);
}

static uint32_t uart_service_notification_count;

/**
 * This count increases each time the hub sends a notification on the nRF UART
...
void pbio_test_bluetooth_send_uart_data(const uint8_t *data, uint32_t size) {
    // nRF UART Rx characteristic value (comes from header file generated by .gatt)
    const uint16_t attribute_handle = 0x000a;
...
void pbio_test_bluetooth_enable_pybricks_service_notifications(void) {
    // client characteristic configuration descriptor (comes from header file generated by .gatt)
    pbio_test_bluetooth_enable_notifications(0x0007);
}
