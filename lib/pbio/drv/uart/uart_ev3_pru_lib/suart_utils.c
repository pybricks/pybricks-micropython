...
static int16_t suart_mcasp_tx_baud_set(uint32_t txBaudValue,
    arm_pru_iomap *pru_arm_iomap);
static int16_t suart_mcasp_rx_baud_set(uint32_t rxBaudValue,
...
uint32_t lt_tx_baud_rate[][SUART_TRX_DIV_CONF_SZ] = {
 ...
 *		AUXCLK = 24MHz
 */
uint32_t lt_rx_8x_baud_rate[][SUART_TRX_DIV_CONF_SZ] = {
/*BaudRate, Divisor, CLKXDIV, HCLKXDIV */
    {300, 10000, 4, 2000},
    {600, 5000, 1, 2500},
    {1800, 1667, 0, 1667},
 ...
 *		AUXCLK = 24MHz
 */
uint32_t lt_rx_16x_baud_rate[][SUART_TRX_DIV_CONF_SZ] = {
/*BaudRate, Divisor, CLKXDIV, HCLKXDIV */
    {300, 5000, 1, 2500},
    {600, 2500, 0, 2500},
    {1800, 833, 0, 833},
...
void suart_mcasp_reset(arm_pru_iomap *pru_arm_iomap) {
    ...
    CSL_McaspRegsOvly mcasp0Regs = (CSL_McaspRegsOvly)pru_arm_iomap->mcasp_io_addr;
    ;                                                                                      // CSL_MCASP_0_CTRL_REGS;
...
void suart_mcasp_config(uint32_t mcasp_addr,
...
void suart_mcasp_tx_serialzier_set(uint32_t serializerNum,
    arm_pru_iomap *pru_arm_iomap) {
 ...
 */
int16_t suart_mcasp_tx_baud_set(uint32_t txBaudValue,
    arm_pru_iomap *pru_arm_iomap) {
    uint32_t clkDivVal;
    uint32_t loopCnt;
    int16_t status = SUART_SUCCESS;
    int16_t foundVal = SUART_FALSE;
 ...
 */
int16_t suart_mcasp_rx_baud_set(uint32_t rxBaudValue,
    uint32_t oversampling,
    arm_pru_iomap *pru_arm_iomap) {
    uint32_t clkDivVal;
    uint32_t loopCnt;
 ...
 */
int16_t suart_asp_baud_set(uint32_t txBaudValue,
    uint32_t rxBaudValue,
    uint32_t oversampling,
    arm_pru_iomap *pru_arm_iomap) {
    int16_t status = SUART_SUCCESS;
 ...
 */
int16_t suart_asp_serializer_deactivate(uint16_t u16srNum,
    arm_pru_iomap *pru_arm_iomap) {
    int16_t status = SUART_SUCCESS;
    CSL_McaspRegsOvly mcasp0Regs =
        (CSL_McaspRegsOvly)pru_arm_iomap->mcasp_io_addr;
    uint32_t *pu32SrCtlAddr = NULL;

    if (u16srNum > 15) {
        status = SUART_INVALID_SR_NUM;
    } else {
        pu32SrCtlAddr = (uint32_t *)&(mcasp0Regs->SRCTL0);
 ...
 */
void suart_mcasp_psc_enable(uint32_t psc1_addr) {
 ...
 */
void suart_mcasp_psc_disable(uint32_t psc1_addr) {
    CSL_PscRegsOvly PSC = (CSL_PscRegsOvly)psc1_addr;           // CSL_PSC_1_REGS;
