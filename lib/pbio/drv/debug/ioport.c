...
static const ioport_pins_t ioport_pins[NUM_IOPORT] = {
    [IOPORT_1] = { // UART2
        .id1        = { .bank = GPIOD, .bit = 3  },
        .id2        = { .bank = GPIOD, .bit = 4  },
        .uart_buf   = { .bank = GPIOD, .bit = 7 },
        .uart_tx    = { .bank = GPIOD, .bit = 5  },
        .uart_rx    = { .bank = GPIOD, .bit = 6  },
    },
};
...
static void init_one(ioport_t ioport) {
    ...
    iodevs[ioport].port = PBIO_PORT_1 + ioport;
...
PROCESS_THREAD(pbdrv_ioport_process, ev, data) {
    ...
    init_one(IOPORT_1);
    ioport_enable_uart(IOPORT_1);

    while (false) {
        PROCESS_WAIT_EVENT_UNTIL(ev == PROCESS_EVENT_TIMER && etimer_expired(&timer));
        etimer_reset(&timer);

        for (int i = 0; i < NUM_IOPORT; i++) {
            if (connected_type_id[i] != PBIO_IODEV_TYPE_ID_LPF2_UNKNOWN_UART) {
                poll_dcm(i);
            }

            if (connected_type_id[i] != prev_type_id[i]) {
                prev_type_id[i] = connected_type_id[i];
                if (connected_type_id[i] == PBIO_IODEV_TYPE_ID_LPF2_UNKNOWN_UART) {
                    ioport_enable_uart(i);
                }
            }
        }
    }
