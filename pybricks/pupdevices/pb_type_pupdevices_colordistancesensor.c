...
STATIC mp_obj_t pupdevices_ColorDistanceSensor_light_on(void *context, const pbio_color_hsv_t *hsv) {
    pb_pupdevices_obj_base_t *sensor = context;
    ...
    return pb_pupdevices_set_data(sensor, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__COL_O, &color);
}
...
STATIC mp_obj_t pupdevices_ColorDistanceSensor_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    pb_pupdevices_init_class(&self->pup_base, port_in, PBIO_IODEV_TYPE_ID_COLOR_DIST_SENSOR);
...
STATIC void get_hsv_data(pupdevices_ColorDistanceSensor_obj_t *self, pbio_color_hsv_t *hsv) {
    int16_t *raw = pb_pupdevices_get_data(&self->pup_base, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__RGB_I);
...
STATIC PB_DEFINE_CONST_PUPDEVICES_METHOD_OBJ(get_color_obj, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__RGB_I, get_color);
...
STATIC PB_DEFINE_CONST_PUPDEVICES_METHOD_OBJ(get_distance_obj, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__PROX, get_distance);

// pybricks.pupdevices.ColorDistanceSensor.reflection
STATIC mp_obj_t get_reflection(mp_obj_t self_in) {
    int16_t *rgb = pb_pupdevices_get_data(self_in, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__RGB_I);
    return mp_obj_new_int((rgb[0] + rgb[1] + rgb[2]) / 12);
}
STATIC PB_DEFINE_CONST_PUPDEVICES_METHOD_OBJ(get_reflection_obj, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__RGB_I, get_reflection);

// pybricks.pupdevices.ColorDistanceSensor.ambient
STATIC mp_obj_t get_ambient(mp_obj_t self_in) {
    int8_t *ambient = pb_pupdevices_get_data(self_in, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__AMBI);
    return mp_obj_new_int(ambient[0]);
}
STATIC PB_DEFINE_CONST_PUPDEVICES_METHOD_OBJ(get_ambient_obj, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__AMBI, get_ambient);
...
STATIC PB_DEFINE_CONST_PUPDEVICES_METHOD_OBJ(get_hsv_obj, PBIO_IODEV_MODE_PUP_COLOR_DISTANCE_SENSOR__RGB_I, get_hsv);
