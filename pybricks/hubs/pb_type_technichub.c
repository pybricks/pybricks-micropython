...
STATIC mp_obj_t hubs_TechnicHub_shutdown(mp_obj_t self_in) {
    pbdrv_reset(PBDRV_RESET_ACTION_POWER_OFF);
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(hubs_cityhub_shutdown_obj, hubs_TechnicHub_shutdown);

STATIC mp_obj_t hubs_TechnicHub_reboot(mp_obj_t self_in) {
    pbdrv_reset(PBDRV_RESET_ACTION_RESET);
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(hubs_cityhub_reboot_obj, hubs_TechnicHub_reboot);

STATIC mp_obj_t hubs_TechnicHub_update(mp_obj_t self_in) {
    pbdrv_reset(PBDRV_RESET_ACTION_RESET_IN_UPDATE_MODE);
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(hubs_cityhub_update_obj, hubs_TechnicHub_update);
