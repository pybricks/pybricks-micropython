...
static mp_obj_t pb_type_Image_make_new(const mp_obj_type_t *type,
    ...
    if (mp_obj_is_type(source_in, &pb_type_Image)) {
    ...
    } else if (mp_obj_is_qstr(source_in) && MP_OBJ_QSTR_VALUE(source_in) == MP_QSTR__screen_) {
        // Screen.
        self->buffer = NULL;
        self->is_display = true;
        self->image = *pbdrv_display_get_image();
        ...
        mp_raise_ValueError(MP_ERROR_TEXT("'_screen_' is the only supported source string"));
    } else {
...
static mp_obj_t pb_type_Image_draw_image(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    if (transparent_in != mp_const_none) {
        transparent = true;
        transparent_value = get_color(transparent_in);
    }
