...
STATIC bool common_Motor_test_completion(void *object, uint32_t end_time) {
    ...
    return pbio_control_is_done(&srv->control);
}

STATIC void common_Motor_cancel(void *object) {
    pbio_servo_t *srv = object;
    pb_assert(pbio_servo_stop(srv, PBIO_CONTROL_ON_COMPLETION_COAST));
}
...
STATIC mp_obj_t common_Motor_reset_angle(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    pb_type_awaitable_cancel_all(self, self->awaitables, PB_TYPE_AWAITABLE_CANCEL_LINKED);
...
STATIC mp_obj_t common_Motor_stall_return_value(void *object) {
