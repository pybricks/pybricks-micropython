...
make -s -j $(nproc --all) -C "$BRICK_DIR" build-test/pybricks-micropython build-test/libgrx-3.0-vdriver-test.so CROSS_COMPILE= DEBUG=1 COPT=-O0 BUILD=build-test

export PYBRICKS_MICROPYTHON="$BUILD_DIR/pybricks-micropython"
